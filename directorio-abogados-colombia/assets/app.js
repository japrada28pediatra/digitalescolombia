...
async function initDirectory(){
 ...
 const data=await fetch(root.dataset.source||'/assets/data.json').then(r=>r.json());
 ...
 list.innerHTML=out.slice(0,120).map(x=>`<article class="card"><h3><a href="/firmas/${x.s}.html">${escapeHTML(x.n)}</a></h3><div class="meta"><span class="pill">${escapeHTML(x.c)}</span><span class="pill ${x.v==='Sí'?'ok':'warn'}">${x.v==='Sí'?'Registro verificado':'Pendiente de verificación'}</span></div><p>${escapeHTML(x.e||'Servicios legales')}</p><div class="card-footer"><span class="small">${escapeHTML(x.t||'Contacto por completar')}</span><a href="/firmas/${x.s}.html">Ver perfil →</a></div></article>`).join('')||'<div class="empty">No encontramos registros con esos filtros.</div>';
 };
